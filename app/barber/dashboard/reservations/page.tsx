...
import { cn, formatTime as formatTimeUtil, nowInIsrael, getIsraelDayStart, getIsraelDayEnd, timestampToIsraelDate, isSameDayInIsrael, getDayKeyInIsrael, normalizeTimestampFormat } from '@/lib/utils'
import { addDays, format, startOfWeek, endOfWeek, isSameDay, parse } from 'date-fns'
import { he } from 'date-fns/locale'
import { Calendar, Plus, ChevronDown, Repeat } from 'lucide-react'
...
function ReservationsContent() {
  ...
  const fetchRecurringAppointments = async () => {
    ...
    try {
      ...
      const displayRecurring: RecurringForDisplay[] = recurringData
        ...
        .filter(rec => rec.time_timestamp > now)
      ...
      if (dateParam) {
        try {
          ...
          if (!isNaN(parsedDate.getTime())) {
            const israelNowDate = nowInIsrael()
            if (isSameDay(parsedDate, israelNowDate)) {
              setQuickDate('today')
              setCustomDate(null)
            } else if (isSameDay(parsedDate, addDays(israelNowDate, 1))) {
  ...
  // Get selected date for timeline and manual booking
  ...
  // Auto-scroll to upcoming section
  ...
  return (
    <div className="max-w-3xl">
      ...
      {/* Date Filter Chips (extracted component) */}
      <DateFilterChips
        quickDate={quickDate}
      ...
      {/* View Mode Selector (extracted component) */}
      <ViewModeSelector
        viewMode={viewMode}
        viewModes={viewModes}
            ...
            {timelineItems.map((item: TimelineItem, index: number) => {
              ...
              return (
                <ReservationRow
                  ...
                  isPast={isPastItem}
                  isHighlighted={highlightedId === res.id}
